...
export async function GetProductData(
  ...
  try {
    const response = await axios.get(
      // `${process.env.NEXT_PUBLIC_WORDPRESS_API_URL}/wp-json/wp/v2/product?include=${productIds.join(",")}`,
      `http://www.orangeui.cn:10030/shopcenter/get_goods_info?appid=1000&user_fid=${userFid}&key=&shop_goods_fid=${productId}`,
    );
...
export async function getUserInfo() {
  ...
  try {
    const response = await axios.get(
      `http://www.orangeui.cn:10030/usercenter/get_user_info?key=${token}`,
    );
