...
export async function saveChatMessage(data: any): Promise<void> {
  ...
  const freeChatState = localStorage.getItem("freeChatExhausted") || "false";
  const url = `/save_api/aicenter/save_chat_message?key=${login_key}`;
  ...
  try {
    ...
    if (loggedIn === "false" && data.sender_temp_user_id === "ai") {
      if (response.data.Code === 200) {
        console.log("[Save Successful]");
        if (freeChatState === "true") {
          localStorage.setItem("freeChatExhausted", "false");
        }
      } else {
        console.log("[Save Failed]: ", response.data.Desc);
        localStorage.setItem("freeChatExhausted", "true");
      }
    }
  } catch (error) {
